...
// local: src\components\ModalNPC.tsx
...
export default function ModalNPC({campaign, npcName, playerName, onClose}: ModalNPCProps) {
  ...
  const handleSend = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(`/api/room/${campaign.room}/adventure/npc`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({action: message, playerName, char: selectedChar, type: 'npc'}),
      });
  ...
  const handleClose = async () => {
    try {
      await fetch(`/api/room/${campaign.room}/adventure/npc`, {method: 'DELETE'});
    }
    finally {
      onClose();
    }
